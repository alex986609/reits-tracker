...
def fetch_sse():
    ...
    try:
        ...
        print("Playwright not installed, using cached SSE data", file=sys.stderr)
        return []
...
def main():
    ...
    cached = None
    try: tzxm_resp = fetch_tzxm(); tzxm_data = tzxm_resp['data']['list'] if isinstance(tzxm_resp.get('data'), dict) else tzxm_resp['data']
    except: print("  tzxm FAILED, using cached"); cached = json.load(open('data.json')); tzxm_data = cached['tzxm']
    print(f"  tzxm: {len(tzxm_data)} entries")
    
    print("Fetching SZSE data...")
    try: szse_ipo, szse_exp = fetch_szse(21), fetch_szse(23)
    except: print("  SZSE FAILED, using cached"); szse_ipo = (cached or json.load(open('data.json')))['szse_ipo']; szse_exp = (cached or json.load(open('data.json')))['szse_exp']
    print(f"  SZSE IPO: {len(szse_ipo)}, EXP: {len(szse_exp)}")
    
    print("Fetching SSE data...")
    try: sse_data = fetch_sse() or []
    except: print("  SSE FAILED, using cached"); sse_data = (cached or json.load(open('data.json'))).get('sse', [])
    print(f"  SSE: {len(sse_data)} entries")
    
    raw = {'tzxm': tzxm_data, 'sse': sse_data, 'szse_ipo': szse_ipo, 'szse_exp': szse_exp}
    json.dump(raw, open('data.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
    
    pool = build_exchange_pool(sse_data, szse_ipo, szse_exp)
    rows, mc, rc = match_and_build(tzxm_data, pool)
    sse_n, szse_n = len(sse_data), len(szse_ipo) + len(szse_exp)
    print(f"Match: {mc}/{len(tzxm_data)} ({round(mc/len(tzxm_data)*100)}%), Reg: {rc}")
    
    html = generate_html(rows, mc, rc, len(tzxm_data), sse_n, szse_n)
